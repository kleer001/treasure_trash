...
| `levels.md` | **The live ruleset** + cell-exact verified rooms (L0–L3) and sketches (L4–L5). Start here for mechanics. |
...
| `spike/` | **Playable prototype** of L0–L3 + a headless verifier. Throwaway, gate-legal, not the real game. Formats and API: `spike/FORMATS.md`. |
...
## Quickstart
- **Play the prototype:** `cd spike && ./run.sh 8000`, then open the URL. It loads ES
  modules, so it needs `http://` — opening the file directly will not work.
  Arrows/WASD or the on-screen d-pad; `U` undo, `R` restart.
- **Check every claim the level files make:** `cd spike && node verify.mjs` → L0–L3 solve
  in provably-minimal par (2/4/7/5), the exit is unoccupied across every reachable state,
  and the trap and refusal counts match what the files declare.
