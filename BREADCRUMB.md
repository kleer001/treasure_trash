...
### Sequential
- [ ] #1 (needs: #9) **The fertility survey.** Output is a map of which piece groups yield solvable,
      ...
      **The size.** multisets of 4 from 11 = C(14,4) = 1001; minus the 210 drawn entirely from
      the seven non-carriers (`c x w b j`, couch, cart) = **791 valid groups** — but see #9:
      once the bin carries a bag it leaves that set and the count becomes **875**. At ~500 random
...
Sort out the recycle bin (#9) — it gates the survey, because a bin that carries a bag is a
bin that leaves the non-carrier set and changes the alphabet the survey draws from. Then the
fertility survey (#1). Random placement sampling on an 8×4; multisets of four drawn
